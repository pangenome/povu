...
void append_record(const bd::VG &g, pt::u32 ref_h_idx,
		   ...
		   const ia::walk_to_alts_map &wta, ir::var_type_e vt,
...
void gen_inv_recs(const bd::VG &g, const ist::st &it_,
		  std::vector<VcfRec> &recs)
{
	...
	for (const auto &[_, v] : it_.get_vertices()) {
		...
		for (auto &[len, alts] : v.get_same_len_alts()) {
			...
			//	     " {}",
			//	     ref_sl.comp_pos(ir::var_type_e::inv),
			//	     ref_h_start);
			//	continue;
			// }

			// std::vector<ist::alt> alts_ =
			//	v.get_alts(alt_h_idx, len);

			// for (const ist::alt &a : v.get_alts(alt_h_idx, len))
			// {
			// }
	...
	for (pt::u32 ref_h_idx : tk.get_ref_haps()) {
		...
		for (const auto &[cxt, min_rov] : d) {
			append_record(g, ref_h_idx, cxt, pvst_vtx_ptr,
				      ...
				      min_rov.get_ins(), ir::var_type_e::ins,
				      recs);

			append_record(g, ref_h_idx, cxt, pvst_vtx_ptr,
				      tk.is_tangled(), min_rov,
				      min_rov.get_subs(), ir::var_type_e::sub,
				      recs);

			append_record(g, ref_h_idx, cxt, pvst_vtx_ptr,
				      tk.is_tangled(), min_rov,
				      min_rov.get_dels(), ir::var_type_e::del,
				      recs);
		}
