...
std::string today()
{
	...
	dateTimeStream << year << std::setw(2) << std::setfill('0') << month
		       << std::setw(2) << std::setfill('0') << day;

	return dateTimeStream.str();
}

void report_time(std::ostream &os, const std::string &fn_name,
...
std::pair<std::size_t, std::size_t> frm_bidirected_idx(std::size_t x,
						       bool has_dummy)
{
	auto foo = [&](std::size_t i)
	{
		return (2 * (i + 1)) + (has_dummy ? 1 : 0);
	};
...
{
	double prog =
		(static_cast<double>(pos) / static_cast<double>(total)) * 100.0;
