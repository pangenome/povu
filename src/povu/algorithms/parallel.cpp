...
bool inspect_trunk(const pst::Tree &st, pt::idx_t ai, pt::idx_t zi)
{
	...
	auto cond_b = [&]() -> bool
	{
		...
		if (branching_vtx ==
		    pc::INVALID_IDX) { // no branching vertex found
			// if (dbg) {
			//   std::cerr << "No branching vertex found from " <<
			//   st.get_vertex(zi).g_v_id() << " to " <<
			//   st.get_vertex(ai).g_v_id() << "\n";
			// }
			return false;
		}
		// else {
		//  if (dbg) {
		//    std::cerr << "Branching vertex found: " <<
		//    st.get_vertex(branching_vtx).g_v_id() << "\n";
		//  }
		//}
	...
	// if (dbg) {
	//   std::cerr << "[DEBUG] Inspecting trunk from " <<
	//   st.get_vertex(zi).g_v_id() << " to " << st.get_vertex(ai).g_v_id()
	//   << "\n"; std::cerr << "b " << cond_b() << "\n"; exit(1);
	// }
		   ...
		   const ptu::tree_meta &tm)
{
	...
	for (pt::idx_t ft_v_idx{}; ft_v_idx < ft.vtx_count(); ft_v_idx++) {
		...
		if (in_branch(st, tm, ft_v) || in_trunk(st, ft_v)) {
			...
			// std::cerr << fn_name << ": Found parallel flubble at
			// " << ft_v.as_str() << "\n";
		}
	}
}
