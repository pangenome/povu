	     ...
	     const std::set<pt::id_t> &to_call_ref_ids)
{
	...
	for (pt::u32 w_idx{}; w_idx < WALK_COUNT; ++w_idx) {
		...
		for (pt::u32 r_idx{}; r_idx < g.ref_count(); r_idx++) {
			...
			for (pt::u32 ref_start : r_starts) {
				...
				pos::hap_traversal ht =
					is_right ? pos::hap_traversal::right
						 : pos::hap_traversal::left;

				pcushion.add_pin(
					w_idx, pos::pin{r_idx, ref_start, ht});
				...
				shared::update_exp(r_idx, w_idx, std::move(at),
						   e);
	...
	/* a pin cushion is valid if has a call refs and RoV is not tangled */
	pcushion.remove_uncallable_walks(to_call_ref_ids);
	if (e.is_tangled() || pcushion.is_empty())
