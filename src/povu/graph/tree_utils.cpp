...
BranchDesc br_desc(const pst::Tree &st)
{
	...
	std::stack<pt::idx_t> edge_stack; // idx of the edge we took could be a
					  // pair with the above

	std::unordered_set<pt::idx_t> in_br_stack;
	br_stack.push(root_idx);

	auto sort_branches = [&](pt::idx_t v_idx)
	{
		pt::idx_t black_e_idx = branch_desc[v_idx].black_e_idx;

		const std::vector<pt::idx_t> &c_e_idxs =
			st.get_child_edge_idxs(v_idx);

		std::vector<pt::idx_t> &temp_sorted =
	...
	while (!s.empty()) {
		...
		if (v.is_leaf()) { // 0 children
			...
			if (is_exp) {
				pt::idx_t top_v_idx = br_stack.top();
 ...
 */
std::vector<pt::idx_t> collect_backedges_by_vertex(
	const pst::Tree &st, const std::vector<pt::idx_t> &B,
	const std::vector<pt::idx_t> &off, std::vector<pt::idx_t> &BE)
{
...
void compute_LoA(const pst::Tree &st, tree_meta &tm)
{
	const std::string fn_name =
		pv_cmp::format("[povu::hubbles::{}]", __func__);
	...
	std::priority_queue<pt::idx_t, std::vector<pt::idx_t>, decltype(cmp)>
...
std::vector<pt::idx_t> count_brackets(const pst::Tree &st,
				      const std::vector<pt::idx_t> &B)
{

	const pt::idx_t n = st.vtx_count();

	std::vector<pt::idx_t> be_count(n,
...
{
	const std::string fn_name =
		pv_cmp::format("[povu::tree_utils::{}]", __func__);
