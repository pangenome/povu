...
pgt::v_end_e Edge::get_v1_end() const
{
	return this->v1_end_;
}
...
pgt::v_end_e Edge::get_v2_end() const
{
	return this->v2_end_;
}
...
pgt::side_n_idx_t Edge::get_other_vtx(pt::idx_t v_idx, pgt::v_end_e ve) const
{
	...
	return (v1 == v_idx) ? pgt::side_n_id_t{v2_end_, v2_idx_}
			     : pgt::side_n_id_t{v1_end_, v1_idx_};
}
...
std::vector<VG *> VG::componetize(const povu::bidirected::VG &g)
{
			     ...
			     pt::idx_t e_idx) -> void
	{
		...
		// handles self loops
		auto [adj_s, adj_v_idx] = e.get_other_vtx(v_idx, ve);
		...
		if (s.empty()) {
			...
			for (auto v_idx_ : comp_vtxs) {
				...
				for (auto e_idx : v_.get_edges_l()) {
					add_edges(v_, pgt::v_end_e::l, v_idx_,
						  e_idx);
				}

				for (auto e_idx : v_.get_edges_r()) {
					add_edges(v_, pgt::v_end_e::r, v_idx_,
						  e_idx);
				}
			}
