...
TEST(PVSTTest, HasVertices) {
  ...
  for (pt::idx_t i = 0; i < pvst.vtx_count(); ++i) {
    ...
    EXPECT_TRUE(expected_labels.find(v.as_str()) != expected_labels.end());
  }
}
...
TEST(PVSTTest, VertexHierarchy) {
  ...
  for (pt::idx_t i = 0; i < pvst.vtx_count(); ++i) {
    ...
    if (v.as_str() == ".") {
      ...
      EXPECT_EQ(pvst.get_vertex(children.front()).as_str(), ">1>7");
    ...
    if (v.as_str() == ">1>7") {
      ...
      EXPECT_EQ(children.size(), 1);
      EXPECT_EQ(pvst.get_vertex(children.front()).as_str(), ">4>6");
    }
