...
| Flubble detection | `detectFlubbles_sound`, `detectFlubbles_complete`, `detectFlubbles_canonical_noDuplicates`, `detectFlubbles_correct`, `flubble_count_le_numEdges`, and `detectFlubbles_correct_for_gfa` prove the Lean reference detector against `IsFlubbleBoundary` and bound the canonical detector output by `edgeCount`. | The current implementation must supply a `CycleClassAssignment.Correct` witness matching povu's bracket/cycle-equivalence state and canonical candidate order. | A graph-only count over an abstract `flubbles G` set remains a future strengthening; the current bound is intentionally over `detectFlubbles frame classes`. |
