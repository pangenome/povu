...
| Reference detector correctness | `PovuLean.Algorithms.Flubble.Correctness` (`PovuLean/Algorithms/Flubble/Correctness.lean`) | `detectFlubbles_sound`, `detectFlubbles_complete`, `detectFlubbles_canonical_noDuplicates`, `detectFlubbles_correct`, `flubble_count_le_numEdges`, `detectFlubbles_correct_for_gfa` | Proves soundness, completeness, duplicate freedom, the paper-facing canonical detector count bound, and the GFA semantic-input corollary for the Lean reference detector. |
