...
struct gt_data {
private:
	std::map<qt::u32, std::vector<at_meta>> data; // allele_to_sample_phase

public:
	void insert(qt::u32 at_idx, const at_meta &ap)
	{
		this->data[at_idx].emplace_back(ap);
	}

	[[nodiscard]]
	const std::map<qt::u32, std::vector<at_meta>> &get_data() const
	{
		return this->data;
	}

	[[nodiscard]]
	std::vector<at_meta> get_meta_for_allele(qt::u32 at_idx) const
	{
...
private:
	...
	void handle_gt(qt::u32 i, const std::string &gt_str, gt_data &a)
	{
		...
		for (qt::u32 j = 0; j < d.size(); j++) {
			...
			// also allele index
			auto at_idx = static_cast<qt::u32>(std::stoul(allele));
			      ...
			      qt::u32 start_col)
	{
		...
		for (qt::u32 i = start_col; i < s.size(); i++) {
...
public:
	...
	[[nodiscard]]
	qt::op_t<ptg::id_or_t> get_ef() const
	{
		return this->ef_;
	}
	...
	[[nodiscard]]
	const std::string &get_at(qt::u32 at_idx) const
	{
		return this->allele_traversals[at_idx];
	}

	[[nodiscard]]
	qt::u32 get_at_count() const
	{
		return this->allele_traversals.size();
	}

	[[nodiscard]]
	qt::u32 get_alt_count() const
	{
		return this->alts.size();
	}

	[[nodiscard]]
	qt::u32 get_pos() const
	{
		return this->pos;
	}
	...
	void viewer(std::ostream &os) const
	{
		...
		auto N = static_cast<qt::u32>(this->allele_traversals.size());
...
struct VCFile {
private:
	pu::TwoWayMap<std::string, qt::u32> sns; // sample name to index
...
public:
	...
	[[nodiscard]]
	qt::u32 record_count() const
	{
		return this->records.size();
	}

	[[nodiscard]]
	qt::u32 sample_count() const
	{
		return this->sns.get_keys().size();
	}

	[[nodiscard]]
	qt::u32 get_sample_idx(const std::string &sample_name) const
	{
		return this->sns.get_value(sample_name);
	}

	[[nodiscard]]
	std::string get_sample_name(qt::u32 sample_idx) const
	{
	...
	std::vector<std::string> get_sample_names() const
	{
		...
		for (qt::u32 i{}; i < this->sample_count(); i++)
