...
public:
	    ...
	    : left(nullptr), right(nullptr)
	{
		...
		const auto minmaxStop = std::minmax_element(
			ivals.begin(), ivals.end(), IntervalStopCmp());
		const auto minmaxStart = std::minmax_element(
			ivals.begin(), ivals.end(), IntervalStartCmp());
		if (!ivals.empty()) {
			center = (minmaxStart.first->start +
				  minmaxStop.second->stop) /
				 2;
		}
		if (leftextent == 0 && rightextent == 0) {
			...
			assert(std::is_sorted(ivals.begin(), ivals.end(),
					      IntervalStartCmp()));
		}
		if (depth == 0 ||
			...
			if (leftextent || rightextent) {
				...
				rightp = std::max_element(ivals.begin(),
							  ivals.end(),
							  IntervalStopCmp())
						 ->stop;
			}
			...
			for (typename interval_vector::const_iterator i =
				     ivals.begin();
			     i != ivals.end(); ++i) {
			...
			if (!lefts.empty()) {
				left.reset(new IntervalTree(
					std::move(lefts), depth, minbucket,
					maxbucket, leftp, center));
			}
			if (!rights.empty()) {
				right.reset(new IntervalTree(
					std::move(rights), depth, minbucket,
	...
	interval_vector findContained(const Scalar &start,
				      const Scalar &stop) const
	{
		interval_vector result;
	...
	std::pair<bool, std::pair<Scalar, Scalar>> is_valid() const
	{
		...
		std::pair<bool, std::pair<Scalar, Scalar>> result = {
			...
			{std::numeric_limits<Scalar>::max(),
			 std::numeric_limits<Scalar>::min()}};
		if (!intervals.empty()) {
			result.second.first = std::min(
				result.second.first, minmaxStart.first->start);
			result.second.second = std::min(
				result.second.second, minmaxStop.second->stop);
		}
		if (left) {
			auto valid = left->is_valid();
			result.first &= valid.first;
			result.second.first = std::min(result.second.first,
						       valid.second.first);
			result.second.second = std::min(result.second.second,
							valid.second.second);
			if (!result.first) {
				return result;
			}
	...
	friend std::ostream &operator<<(std::ostream &os,
					const IntervalTree &itree)
	{
		return writeOut(os, itree);
	}

	friend std::ostream &writeOut(std::ostream &os,
				      const IntervalTree &itree,
				      std::size_t depth = 0)
	{
		auto pad = [&]()
