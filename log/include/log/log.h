...
/// @remark Compile with LOGC__USER_SETTINGS defined to include log_conf.h and
/// be able to modify the above settings
...
typedef struct {
	...
	int line;  /// @var Line in the source file where the log event was
		   /// raised
...
#define log_trace(...)                                                         \
	log_log(LOGC_TRACE, __FILE__, __func__, __LINE__, __VA_ARGS__)
#define log_debug(...)                                                         \
	log_log(LOGC_DEBUG, __FILE__, __func__, __LINE__, __VA_ARGS__)
#define log_info(...)                                                          \
	log_log(LOGC_INFO, __FILE__, __func__, __LINE__, __VA_ARGS__)
#define log_warn(...)                                                          \
	log_log(LOGC_WARN, __FILE__, __func__, __LINE__, __VA_ARGS__)
#define log_error(...)                                                         \
	log_log(LOGC_ERROR, __FILE__, __func__, __LINE__, __VA_ARGS__)
#define log_fatal(...)                                                         \
	log_log(LOGC_FATAL, __FILE__, __func__, __LINE__, __VA_ARGS__)
...
/// @remark         The lock function is passed true to acquire the resource,
/// false to release
...
/// @remark         The callback function will be passed a log_Event structure
/// when called
...
void log_log(log_LogLevel level, const char *file, const char *fn, int line,
	     const char *fmt, ...);
