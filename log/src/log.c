...
/// @brief Define the used time format. 0= none, 1= timestamp, 2= local time
/// formatted
...
struct LogManagemnt {
	...
	Callback callbacks[LOGC__MAX_CALLBACKS]; /// @var Log event callbacks
						 /// list
} LogManagemnt;
...
static void stdout_callback(log_Event *ev)
{
	...
	char buf[16];
	buf[strftime(buf, sizeof(buf), "%H:%M:%S", localtime(&ev->time))] =
		'\0';
...
#ifdef LOGC__STDOUT_COLOR
	fprintf(ev->stream, "%s%s\x1b[0m ", level_colors[ev->level],
		level_strings[ev->level]);

#ifndef LOGC__STDOUT_NO_FILEINFO
	fprintf(ev->stream, "\x1b[90m[%s (%s:%d)]\x1b[0m ", ev->file, ev->fn,
		ev->line);
#endif
#else
	fprintf(ev->stream, "%s ", level_strings[ev->level]);

#ifndef LOGC__STDOUT_NO_FILEINFO
	fprintf(ev->stream, "[%s (%s:%d)] ", ev->file, ev->fn, ev->line);
#endif
#endif
...
#if LOGC__TIME_FORMAT == 2
	char buf[64];
	buf[strftime(buf, sizeof(buf), "%Y-%m-%d %H:%M:%S",
		     localtime(&ev->time))] = '\0';
	fprintf(ev->stream, "%s ", buf);
#elif LOGC__TIME_FORMAT == 1
	fprintf(ev->stream, "%d ", ev->time);
#endif
	fprintf(ev->stream, "%-5s %s:%d: ", level_strings[ev->level], ev->file,
		ev->line);
	vfprintf(ev->stream, ev->fmt, ev->ap);
	fprintf(ev->stream, "\n");
	fflush(ev->stream);
