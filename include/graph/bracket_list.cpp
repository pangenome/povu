...
Bracket::Bracket(std::size_t backedge_id)
    : back_edge_id_(backedge_id), recent_size_(UNDEFINED_SIZE_T),
      recent_class_(UNDEFINED_SIZE_T)
{}

std::size_t Bracket::back_edge_id()
{
	return this->back_edge_id_;
}

std::size_t Bracket::recent_class() const
{
	return this->recent_class_;
}

std::size_t Bracket::recent_size() const
{
	return this->recent_size_;
}

void Bracket::set_recent_size(std::size_t s)
{
	this->recent_size_ = s;
}

void Bracket::set_recent_class(std::size_t c)
{
	this->recent_class_ = c;
}
...
void WBracketList::del(std::size_t be_id)
{
	...
	// TODO: the be_id should always be in the bracket_map, why the need to
	// check?
...
{
	this->brackets.splice(this->brackets.begin(),
			      child->get_bracket_list());

	// update the bracket_map by iterating through the list
	for (auto it = this->brackets.begin(); it != this->brackets.end();
	     ++it) {
