...
public:
  ...
  Tree(pt::idx_t expected_size) : Tree() {
    ...
    this->parent_v = std::vector<pt::idx_t>(expected_size+1, pc::INVALID_ID);
  ...
  template <typename T> pt::idx_t add_vertex(T v) {
    ...
    while (v_idx >= this->parent_v.size()) {
      this->parent_v.push_back(pc::INVALID_ID); // ensure parent_v has enough space
    }
  ...
  void add_edge(pt::idx_t parent, pt::idx_t child) {
    while (child >= this->parent_v.size()) { this->parent_v.push_back(pc::INVALID_ID); }
