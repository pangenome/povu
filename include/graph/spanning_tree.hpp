...
class Tree
{
	...
	// key is the graph_idx and the value is the tree_idx or the backedge
	// idx
	...
	/*
	  a map from the edge id (in the spanning tree) to the index in the
	  tree_edges vector or the back_edges vector key is the edge id and the
	  value is a pair of the edge type and the index in the tree_edges or
	  back_edges vector
	 */
...
public:
	...
	get_obe_w_id(std::size_t vertex);
	std::set<std::pair<std::size_t, std::size_t>>
	get_ibe_w_id(std::size_t vertex);

	// take a vertex index and return an edge id and the child node index
	std::set<std::pair<std::size_t, std::size_t>>
	get_children_w_id(std::size_t vertex);
	...
	// given the back edge's unique back edge id return a reference to the
	// backedge
	...
	// given a vertex id, return a reference to the edge that points to the
	// parent
	...
	std::size_t get_parent(std::size_t v_idx); // TODO: remove, superceded
						   // by get_parent_idx
	...
	// vst
	void concat_bracket_lists(std::size_t parent_vertex,
