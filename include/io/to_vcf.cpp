...
void write_col_header(const std::vector<std::string> &genotype_col_names, std::ostream &os) {
...
*/
void write_rec(const bd::VG &g, pgv::VcfRec &r, const std::string &chrom, std::ostream &os) {
  ...
  auto get_label = [&](const pgt::step_t &step) -> std::string {
    ...
    return (o == pgt::or_e::forward) ? v.get_label() : v.get_rc_label();
  };

  auto slice_to_dna_str = [&](const pga::allele_slice_t &as) -> std::string {
    std::string dna_str = "";
  ...
  auto slices_to_dna_str = [&](const std::vector<pt::idx_t> &idxs) -> std::string {
    std::string dna_strs = "";
    std::string sep = "";
    for (auto idx : idxs) {
  ...
  auto slices_as_at_str = [&](const std::vector<pt::idx_t> &idxs) -> std::string {
    std::string str = "";
...
void write_vcfs(pgv::VcfRecIdx &vcf_recs, const bd::VG &g,
  ...
  for (auto &[ref_id, recs] : vcf_recs.get_recs_mut()) {
    ...
    std::ostream &os = to_stdout ? *stdout_os : vout.stream_for(g.get_sample_name(ref_id));
