...
public:
  ...
  static VcfOutput to_split_files(const std::vector<std::string> &ref_labels,
    ...
    for (const auto &ref_label : ref_labels) {
      ...
      { // if file exists, truncate
        std::ofstream(vcf_fp, std::ios::out | std::ios::trunc);
      }
      std::ofstream ofs(vcf_fp, std::ios::out | std::ios::app);
      if (!ofs) {
        ERR("Append open failed: {}", fs::absolute(vcf_fp).string());
