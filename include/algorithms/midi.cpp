...
std::vector<pvst::MidiBubble> handle_fl(const pst::Tree &st,
  ...
  auto is_desc = [&](pt::idx_t p_v_idx, pt::idx_t c_v_idx) -> bool {
    return ((st.get_vertex(p_v_idx).pre_order() < st.get_vertex(c_v_idx).pre_order()) &&
            (st.get_vertex(p_v_idx).post_order() > st.get_vertex(c_v_idx).post_order()));
  };

  // is v_idx in the trunk? true if v_idx is an ancestor of zi, then it is in the trunk
  auto in_trunk = [&](pt::idx_t v_idx) -> bool { return v_idx < zi; };
    ...
    if (in_trunk(g_or_s_st_idx)) {
      ...
      for (auto p_v_idx : c) {
        if (is_desc(p_v_idx, g_or_s_st_idx)) {
          branch_c_bubs[p_v_idx].push_back(g_or_s_st_idx);
        }
      }
    }
  }
