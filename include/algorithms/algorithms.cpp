...
const boundary NULL_BOUNDARY{.b1 = pc::UNDEFINED_SIZE_T, .b2 = pc::UNDEFINED_SIZE_T};
  ...
  if (report_time && v % 10000 == 0) {
    ...
    if (t.get_bracket_list(v).empty()) {
      ...
      if (t.get_vertex(v).type() != v_type_e::dummy) {
        //std::cerr << "add art be " << t.get_vertex(v).name() << " " << dest_v << std::endl;
        if (curr_bry.b1 != pc::UNDEFINED_SIZE_T) { std::cerr << fn_name << "WARN: curr boundary already set\n"; }
