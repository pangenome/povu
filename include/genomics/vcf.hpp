...
class VcfRec {
  ...
  std::map<pt::idx_t, pt::idx_t> col_to_allele_count_; // genotype column idx to allele count
  ...
  void comp_unique_alt_ats() {
    std::map<std::tuple<pt::idx_t, pt::idx_t, pt::idx_t>, pt::idx_t> seen;
    for (pt::idx_t i{}; i < this->ats_.size(); ++i) {
      const auto &alt_at = this->ats_.at(i);
      auto key = std::make_tuple(alt_at.walk_idx, alt_at.walk_start_idx, alt_at.len);
...
public:
  ...
  VcfRec(pt::id_t ref_id, pt::idx_t pos, std::string id,
         pga::allele_slice_t ref_at, pt::idx_t height, var_type_e var_typ, bool is_tangled,
         pt::idx_t ref_at_ref_count,
         std::vector<std::vector<std::string>> &&genotype_cols)
    : ref_id_(ref_id), pos_(pos), id_(id), ats_({ref_at}), height_(height),
        var_type_(var_typ), is_tangled_(is_tangled),
  ...
  VcfRec &operator=(VcfRec &&) = delete; // or = default if you later allow it
  ~VcfRec() = default;
  ...
  const std::string &get_qual() const { return this->qual; }
  const std::string &get_filter() const { return this->filter; }
  const std::string &get_format() const { return this->format; }
  const pga::allele_slice_t &get_ref_at() const { return this->ats_.at(0); }
  ...
  pt::idx_t get_ns() const {
    ...
    pt::idx_t ns{0};
    for (const auto &[_, allele_count] : this->col_to_allele_count_) {
      ns += allele_count;
    }
    return ns;
  }
  ...
  // setter(s)
  // ---------
  pt::idx_t append_alt_at(const pga::allele_slice_t &alt_at, pt::idx_t alt_at_ref_count ) {
...
public:
  ...
  void ensure_append_recs(std::map<pt::idx_t, std::vector<VcfRec>> &&new_recs) {
