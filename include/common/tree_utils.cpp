...
BranchDesc br_desc(const pst::Tree &st) {
  ...
  auto sort_branches = [&](pt::idx_t v_idx) {
    ...
    const std::vector<pt::idx_t> &c_e_idxs = st.get_child_edge_idxs(v_idx);

    std::vector<pt::idx_t> &temp_sorted = branch_desc[v_idx].sorted_br;
  ...
  while(!s.empty()) {
    ...
    if (v.is_leaf()) { // 0 children
      ...
      if (is_exp) {
        ...
        branch_desc[br_stack.top()].edge_data.insert(std::make_pair(e, top_v_idx));
