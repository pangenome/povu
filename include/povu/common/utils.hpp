...
std::string concat_with(const Container &v, char delim)
{
 ...
 * @brief
 */
void report_time(std::ostream &os, std::string fn_name, std::string action,
 ...
 * @brief
 *
 * @param v: the vector whose value is to be erased passed by copy to avoid
...
public:
	...
	Value get_value(const Key &key) const
	{
		...
		// Return a default-constructed Value or throw an exception,
	...
	Key get_key(const Value &value) const
	{
		...
		// Return a default-constructed Key or throw an exception,
		// depending on your use case
		return Key{};
	}
...
/**
 * @brief given a bi-Directed index return the indexes of the left and right
 * vertices in the bi-Edged graph
 *
 * @param
 * @param
 * @return
 */
std::pair<std::size_t, std::size_t> frm_bidirected_idx(std::size_t bd_idx,
						       bool has_dummy = true);
